document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    contactForm.addEventListener('submit', function (e) {
      const fields = contactForm.querySelectorAll('input:not([type="hidden"]):not([type="submit"]):not([type="button"]):not([type="reset"]):not([type="checkbox"]):not([type="radio"]), textarea, select');
      let isValid = true;
      ...
      // Validate checkboxes and radios with required attribute
      const requiredCheckboxes = contactForm.querySelectorAll('input[type="checkbox"][required]');
      requiredCheckboxes.forEach(function (checkbox) {
        if (!checkbox.checked) {
          showError(checkbox, getFieldLabel(checkbox) + ' must be checked.');
          isValid = false;
      ...
      const radioGroups = {};
      contactForm.querySelectorAll('input[type="radio"][required]').forEach(function (radio) {
        if (!radioGroups[radio.name]) radioGroups[radio.name] = [];
        radioGroups[radio.name].push(radio);
      });
      Object.keys(radioGroups).forEach(function (name) {
        const group = radioGroups[name];
        const checked = group.some(function (r) { return r.checked; });
        if (!checked) {
          showError(group[0], 'Please select an option.');
          isValid = false;
          if (!firstInvalidField) firstInvalidField = group[0];
        } else {
          group.forEach(function (r) { clearError(r); });
        }
      });
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'bb700e86-ae83-441f-bf29-c14068b56bc5',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'bb700e86-ae83-441f-bf29-c14068b56bc5';
...
(function () {
  const faqItems = document.querySelectorAll('.hw-faq__item');

  faqItems.forEach(function (item) {
    const btn = item.querySelector('.hw-faq__question');
    ...
    btn.addEventListener('click', function () {
      ...
      faqItems.forEach(function (el) {
        ...
        el.querySelector('.hw-faq__question').setAttribute('aria-expanded', 'false');
      });
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'bb700e86-ae83-441f-bf29-c14068b56bc5';
